...
const UserRegister = () => {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    const { name, email, sub } = decoded as { name: string; email: string; sub: string };
  ...
  return (
    <GoogleOAuthProvider clientId={"1086489884784-2e3p2a6e32tc8ms6arpusk39fi0rm8ib.apps.googleusercontent.com"}>
      <div className="flex h-screen w-full bg-no-repeat bg-cover " style={{ backgroundImage: bgImage }}>
              ...
              {({ isSubmitting }) => (
                <Form className="space-y-4">
                  <div>
                    ...
                    <Field type="text" name="name" className="w-full px-3 py-2 border bg-[#B0BAC3] opacity-40 rounded-[50px]" />
                    <ErrorMessage name="name" component="div" className="text-red-500 text-sm" />
                  </div>

                  <div>
                    <label className="block text-black">Email</label>
                    <Field type="email" name="email" className="w-full px-3 py-2 border bg-[#B0BAC3] opacity-40 rounded-[50px]" />
                    <ErrorMessage name="email" component="div" className="text-red-500 text-sm" />
                  </div>

                  <div className="flex justify-center">
                    <button type="submit" disabled={isSubmitting} className="w-1/2 bg-[#105132bd] text-white py-2 rounded-lg">
