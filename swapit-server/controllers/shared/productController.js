    ...
    if (req.body.latitude && req.body.longitude && req.body.address) {
      ...
      if (!userId) {
        return res
          ...
          .json({
            success: false,
            message: "User must be logged in to add a used product",
          });
      }
      seller = userId;
    }
...
export const getAllProducts = async (req, res) => {
  try {
    ...
    // Only exclude current user's products if logged in
    if (currentUserId) {
      filter.seller = { $ne: new mongoose.Types.ObjectId(currentUserId) };
    }
...
export const getSpecificProduct = async (req, res) => {
  ...
  try {
    ...
    if (!product || (Array.isArray(product) && product.length === 0)) {
      return res
        .status(404)
        .json({ success: false, message: "Product(s) not found" });
    }
